...
async def process_news_worker(worker_id: int, queue: asyncio.Queue):
    ...
    while True:
        ...
        # Determine source type for deduplication
        source_type = "TWITTER" if source.startswith("twitter_") else "TELEGRAM"
        
        try:
            # Check for duplicate before processing
            if await logger.is_duplicate(headline, window_minutes=5):
                sys_logger.info(f"Duplicate detected (within 5 min window). Skipping: {headline[:50]}...")
                queue.task_done()
                continue
            ...
            # 4. Log to SQLite
            event_id = await logger.log_event(text=headline, source_channel_id=source, source_type=source_type)
...
@click.option('--source', default='both', type=click.Choice(['telegram', 'twitter', 'both']), help='News source to use')
def watch(live, workers, source):
    ...
    async def run_pipeline():
        ...
        # Start news streams based on source option
        streams = []
        
        if source in ('telegram', 'both'):
            telegram_stream = NewsStream(queue)
            streams.append(telegram_stream)
            sys_logger.info("Telegram stream enabled.")
        
        if source in ('twitter', 'both'):
            twitter_stream = TwitterStream(queue)
            streams.append(twitter_stream)
            sys_logger.info("Twitter stream enabled.")
        
        if not streams:
            sys_logger.error("No streams enabled. Exiting.")
            return
        
        sys_logger.info(f"Pipeline initialized with {workers} workers. Listening for news...")
        
        # Run all streams concurrently
        stream_tasks = [asyncio.create_task(s.start()) for s in streams]
